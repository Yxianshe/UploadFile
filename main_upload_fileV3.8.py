...
class SFTPUploaderApp:
    def __init__(self, root):
        ...
        self.calc_size = tk.BooleanVar(value=False)
        ...
        try:
            icon_path = resource_path( "UploadFile.ico")
            # 使用 root.iconbitmap() 来设置 .ico 文件作为窗口图标
            if os.path.exists(icon_path):
                self.root.iconbitmap(icon_path)
            else:
                print(f"Icon file not found at: {icon_path}")
        except tk.TclError as e:
            # 如果在非 Windows 系统上，或者 .ico 文件有问题，可能会捕获到 TclError
    ...
    def _init_ui(self):
        ...
        chk_calc_size = tk.Checkbutton(status_frame, text="计算文件大小", variable=self.calc_size, 
                                       bg=COLORS["bg"], fg=COLORS["text_dim"], 
                                       selectcolor=COLORS["input_bg"], activebackground=COLORS["bg"], 
                                       activeforeground=COLORS["accent"], font=("Microsoft YaHei UI", 9))
        chk_calc_size.pack(side="right", padx=10)
    ...
    def run_process(self):
        try:
            ...
            if self.calc_size.get():
                ...
                self.log("跳过文件大小计算 (Skip size calculation)", "INFO")
    ...
    def update_status(self, current_file_name, chunk_size=0):
        ...
        if now - self.last_update_time > 0.5:
            ...
            if self.total_task_size > 0:
                ...
                status_text = f"已传: {mb_transferred:.1f} MB | 速度: {speed_mb:.1f} MB/s | 文件: {current_file_name[-20:]}"
